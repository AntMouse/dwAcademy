...
public class Chap9QProject2Copy {
	...
	public void manageDinosaurs() {
		...
		case 1: // 1. 공룡 정보 추가		
			...
			String newDinoType = parkAdministration.setDinoType("add"); // 공룡 타입 정하기(익룡, 어룡 등)
		...
		case 4: // 4. 공룡 정보 수정
		    ...
		    String changeDinoType = parkAdministration.setDinoType("change"); // 타입 수정   
		    // setDinoType 메서드 실행. 수정하지 않으면 ""으로 공백 반환. 수정할 시 공룡 타입 반환.
		    // 즉, 수정을 했다면 무조건 공룡 종류 중 1개가 나온다.
		    // 또한 빈값을 반환했다면 아래의 allChangeDino 메서드에서 타입이 바뀌지 않는다.
		    // 스위치 문에서 공룡 타입 3개 중 하나가 아니라면 값이 바뀌지 않도록 코드를 했음.
		    int changeDinoIntegerValue;
		    if (!changeDinoType.isEmpty()) { // 비어있지 않다는 건 공룡 종류 1개를 반환했다는 뜻.
		    	changeDinoIntegerValue = parkAdministration.changeDinoTypeValue(changeDinoType);
			} else {
				changeDinoIntegerValue = -1;
			} // 이렇게 -1로 하면 allChangeDino에서 타입 속성 값도 바뀌지 않는다.
