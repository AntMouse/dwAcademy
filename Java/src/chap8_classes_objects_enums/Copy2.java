...
public class Copy2 {
	...
	public void manageDinosaurs() {
		...
		case 1:
			...
			// 인덱스 유효범위 검사
			boolean checkDinoIndex = parkTotalManagement.checkDinoIndex("add", newDinoIndex);	
			if (!checkDinoIndex) {
				break;
			}
			
			System.out.print("추가할 공룡의 이름을 입력하세요 : ");
		...
		case 2:
			System.out.print("조회할 공룡의 관리 번호를 입력하세요(전체 공룡 조회는 -1) : ");
		...
		case 4:
			...
			// 인덱스 유효범위 검사
			// checkDinoIndex 변수는 case 1에서 boolean으로 변수를 선언했다
			checkDinoIndex = parkTotalManagement.checkDinoIndex("change", changeDinoIndex);	
			if (!checkDinoIndex) {
				break;
			}
