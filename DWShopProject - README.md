...
18-1. ORM과 쿼리문에 대해서 제대로 인지를 못해서 시간을 버렸다. 그래도 오늘 일로 ORM, 쿼리문의 사용에 대해 큰 방향을 잡았다. 오늘 3시간 30분 정도를 버렸지만, 앞으로 이런 실수를 안 하게 될 것이라는 점을 생각해보면 이런 식으로 실수도 하면서 하지 말아야 할 행동을 하나씩 제거해나갈 수 있다. 긍정적으로 생각하면 날렸다고 생각한 그 시간도 결국 의미가 있었던 것 같다.



19. 2024-07-02

19-1. 백엔드에서 패키지를 확장성을 고려해서 만드니까 패키지가 많아진다. 그래도 이렇게하면 클래스나 다른 기능이 늘어나도, 이전보다는 관리가 쉬워질 것 같다. 다만, 그것과는 별개로 패키지가 많아지므로 불편한 부분도 분명히 있다. 각각 장단점이 있다. 그래도 나한테는 이 방식이 맞는 것 같다.



20. 2024-07-03

20-1. 중복되는 코드를 하나로 통합해서 쓰거나, 인터페이스에 옮기고, 이를 상속해서 사용하는 경우 등, 중복에 관해서 여러 가지 방법이 있다. 이때, 처음부터 중복을 하나로 합쳐서 쓰려고 하지는 말고, 일단 처음에는 비효율적이더라도 하나의 코드만 먼저 틀을 잡고 작성을 해두자. 그리고 중복되는 코드도 마찬가지로 똑같이 작성을 해두고 해당 파일을 저장을 해둔다. 이제부터 그 다음, 하나씩 중복을 하나로 합치고, 효율성을 생각해서 조금씩 수정해 나가자.

처음부터 중복을 없애겠다고, 완성도 제대로 안 된 상태에서 합치려고 하면, 나중에 오류가 나기도 하고, 무엇보다 이 과정이 실패했을 때 나한테 남는 게 아무것도 없다. 비효율, 중복을 감수하고서라도 일단 완성을 해놓고 수정을 하면 그래도 마음이 조금 더 편한한 상태에서 작업을 할 수 있다.



21. 2024-07-19

21-1. 그동안은 책을 보면서 코딩 공부를 했는데, 이번에 프로젝트를 하면서 코딩 공부를 어떻게, 어떤 방식으로 해야 하는지 어느 정도 감을 잡았다. 앞으로 이 방식대로 공부를 하면 조금씩 실력을 높일 수 있을 것 같다. 아마, 학원을 다니면서 얻은 가장 큰 수확이 이것인 것 같다. 직접 코드를 자신이 손으로 입력을 해봐야 한다는 말을 많이 들었고, 실제로도 그렇게 했는데, 지금은 왜 그렇게 해야 하는지 조금은 깨달을 수 있었다.



22. 2024-07-30

22-1. 이번에 user, address 관련으로 실수를 한 부분이 많았다. 식별 id를 너무 신경쓰다가 코드를 잘못 작성했다. address를 저장하고, 그 자료를 다시 user에 넣고, user를 저장했다. 그러나 이렇게 할 필요 없이, 그냥 user를 먼저 저장을 하고, 그 다음에 user에서 address를 빼와서 그걸 따로 레퍼지토리에 저장을 하면 쉽게 해결할 수 있었다. 사실 이 부분은 당연한 것이기도 하고, 테스트를 하면 나도 금방 알 수 있었을 텐데, 코드를 전체적으로 다 수정을 하다보니 테스트를 하는 게 여의치 않아서 실행을 하지 못했다.

사실상 앞으로 이런 부분은 없긴 하겠지만, 만약을 위해 언제든지 테스트를 할 수 있게 미리 백엔드 환경을 만들어 두고, 코드만 옮겨서 테스트용 백엔드에서 테스트를 할 수 있게 코드를 만들어두는 것도 생각해보자.
