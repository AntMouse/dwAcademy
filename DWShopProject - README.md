...
1. 백엔드에서 패키지를 확장성을 고려해서 만드니까 패키지가 많아진다. 그래도 이렇게하면 클래스나 다른 기능이 늘어나도, 이전보다는 관리가 쉬워질 것 같다. 다만, 그것과는 별개로 패키지가 많아지므로 불편한 부분도 분명히 있다. 각각 장단점이 있다. 그래도 나한테는 이 방식이 맞는 것 같다.
