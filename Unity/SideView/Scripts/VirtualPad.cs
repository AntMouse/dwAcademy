...
{
    public float MaxLength = 70; // 탭이 우직이는 최대 거리
    ...
    public void PadDrag()
    {
        ...
        if (is4DPad == false)
        {
            newTabPos.y = 0; // 횡스크롤일 때는 Y 값을 0으로 한다.
        }
        // 이동 벡터 계산하기
        Vector2 axis = newTabPos.normalized; // 벡터 정규화, 벡터에서 방향 없이 속도만 받기 위해 사용
        // 두 점의 거리 구하기
        float len = Vector2.Distance(defPos, newTabPos);
        if (len > MaxLength)
        {
            // 한계 거리를 넘겼기 때문에 한계 좌표로 설정
