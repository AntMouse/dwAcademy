...
function CartPage({ currentMember }) {
  const { cartId, memberId } = useParams();
  ...
  const updateCart = async (productId, quantity, method) => {
    const url = `${apiUrl}/api/carts/${cartId || currentMember.id}/items`;
    const params = { productId, quantity };
    try {
      await axios({ method, url, params });
      fetchCart(cartId || currentMember.id);
    } catch (error) {
  ...
  return (
    <div className="container">
      <h1>장바구니</h1>
      <div>회원 ID: {cart.memberId || memberId}</div>
      <div>장바구니 ID: {cart.id}</div>
