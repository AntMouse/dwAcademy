...
function ProductList({
  ...
  const minimumRows = 5; // 최소 5개의 행이 표시되도록 설정
  const additionalRows = minimumRows - currentProducts.length;
      ...
      <tbody>
        {currentProducts.map(product => (
          <tr key={product.id} className="product-row">
            ...
            <td className="product-edit-buttons">
              {editProductId === product.id ? (
                <>
        ...
        {/* 빈 행 추가 */}
        {additionalRows > 0 && Array.from({ length: additionalRows }).map((_, index) => (
          <tr key={`empty-${index}`} className="product-row">
            <td className="product-field">
              <div className="product-main-type"></div>
            </td>
            <td className="product-field">
              <div className="product-sub-type"></div>
            </td>
            <td className="product-field">
            ...
            <td className="product-edit-buttons">
              <button className="invisible-button">수정</button>
              <button className="invisible-button">삭제</button>
            </td>
          </tr>
        ))}
      </tbody>
