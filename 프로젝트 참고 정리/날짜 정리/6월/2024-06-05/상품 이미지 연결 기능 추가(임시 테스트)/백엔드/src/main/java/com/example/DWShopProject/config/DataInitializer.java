...
public class DataInitializer implements CommandLineRunner {
    ...
    @Autowired
    private ProductTypeMgmtRepository productTypeMgmtRepository; // ProductType 저장소 추가
    ...
    public void run(String... args) throws Exception {
        ...
        for (ProductTypeEnum type : ProductTypeEnum.values()) {
            for (int i = 1; i <= 3; i++) {
                Product product = Product.builder()
                        ...
                        .productImgUrl("products/" + type.getParentTypeEnum().name().toLowerCase() + "/" + type.name().toLowerCase() + "/image" + i + ".jpg") // 이미지 URL 소문자로 수정
        ...
        for (int i = 1; i <= 7; i++) {
            MemberDto memberDto = MemberDto.builder()
                    .memberType(MemberRoleEnum.ROLE_USER) // 반드시 설정
                    .memberId("user" + i)
                    .memberName("User " + i)
