...
public class AuthService {
    ...
    // Admin 로그인 메서드 추가
    @Transactional
    public String adminLogin(LoginDto loginDto, HttpServletResponse response) {
        log.debug("Admin login attempt with accountId: {}", loginDto.getAccountId());

        Optional<SuperAdmin> optionalAdmin = superAdminRepository.findByAccountId(loginDto.getAccountId());

        if (optionalAdmin.isEmpty()) {
            log.error("관리자가 존재하지 않음: {}", loginDto.getAccountId());
            throw new IllegalArgumentException("관리자가 존재하지 않음");
        }

        SuperAdmin admin = optionalAdmin.get();

        if (!passwordEncoder.matches(loginDto.getPassword(), admin.getPassword())) {
            log.error("비밀번호가 일치하지 않음: {}", loginDto.getAccountId());
            throw new IllegalArgumentException("비밀번호가 일치하지 않습니다.");
        }

        String token = jwtUtil.createToken(admin.getAccountId(), admin.getAccountType());
