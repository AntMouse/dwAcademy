...
public class AuthController {
    ...
    public ResponseEntity<String> adminLogin(@RequestBody LoginDto loginDto, HttpServletResponse response) {
        log.debug("Admin login request received with accountId: {}", loginDto.getAccountId());
        try {
            String token = authService.adminLogin(loginDto, response);
            log.debug("Admin login successful, token generated: {}", token);
            return ResponseEntity.ok(token);
        } catch (IllegalArgumentException e) {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("관리자 로그인 중 오류가 발생했습니다. 다시 시도해주세요.");
        }
    }
