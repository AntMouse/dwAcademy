...
public class UserController {
    ...
    public ResponseEntity<?> signUp(@RequestBody UserDto userDto) {
        ...
        try {
            if (optionalUser.isPresent()) {
                ...
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("하나의 이메일에 하나의 아이디만 만들 수 있습니다.");
            }
    ...
    public ResponseEntity<UserDto> getMyPage(@AuthenticationPrincipal AccountDetailsImpl userDetails) {
        ...
        if (account instanceof User) {
            ...
            return ResponseEntity.ok(myPageDTO); // 수정된 부분: UserDto 반환
        } else {
    ...
    public ResponseEntity<List<String>> getUserTypes() {
        List<String> userTypes = Arrays.stream(ApplicationRoleEnum.values())
                .map(ApplicationRoleEnum::toValue)
                .collect(Collectors.toList());
