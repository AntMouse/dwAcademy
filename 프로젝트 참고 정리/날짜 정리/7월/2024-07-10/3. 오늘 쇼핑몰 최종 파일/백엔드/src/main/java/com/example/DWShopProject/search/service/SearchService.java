...
public class SearchService {
    ...
    public SearchResultDto searchAll(String keyword) {
        ...
        Set<ProductType> foundSubTypes = new HashSet<>(); // 검색된 서브 타입 저장 (중복 방지용)
     ...
     * @return 검색된 상품 리스트
     */
    private List<Product> searchBySubTypeDisplayName(String displayName, Set<ProductType> foundSubTypes) {
     ...
     * @return 검색된 상품 리스트
     */
    private List<Product> searchBySubTypeAndMainType(String keyword, Set<ProductType> foundSubTypes) {
        ...
        List<ProductType> additionalSubTypes = new ArrayList<>();
        for (ParentType parentTypeEnum : ParentType.values()) {
            if (parentTypeEnum.getDisplayName().contains(keyword)) {
                ...
                for (ProductTypeMgmt productTypeMgmt : productTypeMgmtList) {
                    ProductType subType = productTypeMgmt.getProductType();
