package com.example.DWShopProject.searchmanagement.service;

import com.example.DWShopProject.searchmanagement.dto.SearchDto;
import com.example.DWShopProject.searchmanagement.dto.SearchResultDto;
import com.example.DWShopProject.productmanagement.entity.Product;
import com.example.DWShopProject.productmanagement.repository.ProductRepository;
...
public class SearchService {
    ...
    public SearchResultDto searchAll(String keyword) {
        ...
        Set<ProductSubType> foundSubTypes = new HashSet<>(); // 검색된 서브 타입 저장 (중복 방지용)
     ...
     * @return 검색된 상품 리스트
     */
    private List<Product> searchBySubTypeDisplayName(String displayName, Set<ProductSubType> foundSubTypes) {
        return Arrays.stream(ProductSubType.values())
     ...
     * @return 검색된 상품 리스트
     */
    private List<Product> searchBySubTypeAndMainType(String keyword, Set<ProductSubType> foundSubTypes) {
        ...
        List<ProductSubType> additionalSubTypes = new ArrayList<>();
        for (ProductMainType parentTypeEnum : ProductMainType.values()) {
            if (parentTypeEnum.getDisplayName().contains(keyword)) {
                ...
                for (ProductTypeMgmt productTypeMgmt : productTypeMgmtList) {
                    ProductSubType subType = productTypeMgmt.getProductType();
    ...
    private SearchDto mapToDto(Product product) {
        return SearchDto.builder()
                .id(product.getId())
                .productType(product.getProductSubType())
